...
{
	public class FeesCategoryMasterService : IFeesCategoryMasterService
	{
		private FeesCategoryMaster Map(DataRow row)
		{
			if (row == null) return new FeesCategoryMaster();

			var entity = new FeesCategoryMaster();
			if (row.Table.Columns.Contains("Id") && Guid.TryParse(row["Id"].ToString(), out var id)) entity.Id = id;
			entity.FeesCatgoryName = row.Table.Columns.Contains("FeesCatgoryName") ? row["FeesCatgoryName"].ToString() ?? string.Empty : string.Empty;
		...
		public List<FeesCategoryMaster> GetAll()
		{
			Proc p = new Proc("FeesCategoryMaster_GetAll");
			var dt = new DataTable();
			p.Exec(dt);
			var list = new List<FeesCategoryMaster>();
		...
		public FeesCategoryMaster? GetById(Guid id)
		{
			Proc p = new Proc("FeesCategoryMaster_GetById");
			p["@Id"] = id;
			var dt = new DataTable();
			p.Exec(dt);
		...
		public Guid Create(FeesCategoryMaster category)
		{
			Proc p = new Proc("FeesCategoryMaster_Create");
			p["@FeesCatgoryName"] = category.FeesCatgoryName;
		...
		public bool Update(FeesCategoryMaster category)
		{
			Proc p = new Proc("FeesCategoryMaster_Update");
			p["@Id"] = category.Id;
			p["@FeesCatgoryName"] = category.FeesCatgoryName;
			p["@Description"] = category.Description ?? string.Empty;
			p["@IsActive"] = category.IsActive;
		...
		{
			Proc p = new Proc("FeesCategoryMaster_Delete");
			p["@Id"] = id;
			p.Exec();
			var ret = p.Parameters["@RETURN_VALUE"].Value;
			int code = ret == null || ret == DBNull.Value ? 0 : Convert.ToInt32(ret);
