...
namespace SchoolPortal.Services
{
    ...
    {
        private static BookTypeMaster MapBookType(DataRow r)
        {
            var c = new BookTypeMaster();
        ...
        public List<BookTypeMaster> GetAll()
        {
            var list = new List<BookTypeMaster>();
            Proc p = new Proc("BookType_GetAll");
            var dt = new DataTable();
            p.Exec(dt);
            foreach (DataRow r in dt.Rows)
            {
                list.Add(MapBookType(r));
            }
            return list;
        }

        public BookTypeMaster? GetById(Guid id)
        {
            Proc p = new Proc("BookType_GetById");
            p["@Id"] = id;
            var dt = new DataTable();
            p.Exec(dt);
            if (dt.Rows.Count == 0) return null;
            return MapBookType(dt.Rows[0]);
        }

        public Guid Create(BookTypeMaster bookType)
        {
            Proc p = new Proc("BookType_Create");
            p["@Name"] = bookType.Name;
            p["@Description"] = bookType.Description ?? string.Empty;
            p["@IsActive"] = bookType.IsActive;
            p["@CompanyId"] = bookType.CompanyId;
            p["@SchoolId"] = bookType.SchoolId;
            p["@CreatedBy"] = bookType.CreatedBy;
            var dt = new DataTable();
            p.Exec(dt);
            if (dt.Rows.Count > 0)
            {
        ...
        public bool Update(BookTypeMaster bookType)
        {
            Proc p = new Proc("BookType_Update");
            p["@Id"] = bookType.Id;
            p["@Name"] = bookType.Name;
            p["@Description"] = bookType.Description ?? string.Empty;
            p["@IsActive"] = bookType.IsActive;
            p["@CompanyId"] = bookType.CompanyId;
            p["@SchoolId"] = bookType.SchoolId;
            p["@ModifiedBy"] = bookType.ModifiedBy ?? Guid.Empty;
            p.Exec();
        ...
        {
            Proc p = new Proc("BookType_Delete");
            p["@Id"] = id;
            p.Exec();
            var ret = p.Parameters["@RETURN_VALUE"].Value;
            int code = ret == null || ret == DBNull.Value ? 0 : Convert.ToInt32(ret);
        ...
        public string BookTypeNameById(Guid id)
        {
            Proc p = new Proc("BookType_GetById");
            p["@Id"] = id;
            var dt = new DataTable();
