...
{
	public class CleanerDocumentDetailsService : ICleanerDocumentDetailsService
	{
		private static CleanerDocumentDetails Map(DataRow r)
		{
			var e = new CleanerDocumentDetails();
		...
		public List<CleanerDocumentDetails> GetAll()
		{
			var list = new List<CleanerDocumentDetails>();
			Proc p = new Proc("CleanerDocumentDetails_GetAll");
		...
		public CleanerDocumentDetails? GetById(Guid id)
		{
			Proc p = new Proc("CleanerDocumentDetails_GetById");
			p["@Id"] = id;
			var dt = new DataTable();
			p.Exec(dt);
		...
		public Guid Create(CleanerDocumentDetails e)
		{
			Proc p = new Proc("CleanerDocumentDetails_Create");
		...
		public bool Update(CleanerDocumentDetails e)
		{
			Proc p = new Proc("CleanerDocumentDetails_Update");
			p["@Id"] = e.Id;
			p["@CleanerId"] = e.CleanerId;
			p["@Name"] = e.Name ?? string.Empty;
			p["@Description"] = e.Description ?? string.Empty;
		...
		{
			Proc p = new Proc("CleanerDocumentDetails_Delete");
			p["@Id"] = id;
			p.Exec();
			var ret = p.Parameters["@RETURN_VALUE"].Value;
			int code = ret == null || ret == DBNull.Value ? 0 : Convert.ToInt32(ret);
