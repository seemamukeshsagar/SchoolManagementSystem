...
{
	public class TeacherDocumentDetailsService : ITeacherDocumentDetailsService
	{
		private static TeacherDocumentDetails Map(DataRow r)
		{
			var e = new TeacherDocumentDetails();
		...
		public List<TeacherDocumentDetails> GetAll()
		{
			var list = new List<TeacherDocumentDetails>();
			Proc p = new Proc("TeacherDocumentDetails_GetAll");
		...
		public TeacherDocumentDetails? GetById(Guid id)
		{
			Proc p = new Proc("TeacherDocumentDetails_GetById");
			p["@Id"] = id;
			var dt = new DataTable();
			p.Exec(dt);
		...
		public Guid Create(TeacherDocumentDetails e)
		{
			Proc p = new Proc("TeacherDocumentDetails_Create");
		...
		public bool Update(TeacherDocumentDetails e)
		{
			Proc p = new Proc("TeacherDocumentDetails_Update");
			p["@Id"] = e.Id;
			p["@TeacherId"] = e.TeacherId;
			p["@Name"] = e.Name ?? string.Empty;
			p["@Description"] = e.Description ?? string.Empty;
		...
		{
			Proc p = new Proc("TeacherDocumentDetails_Delete");
			p["@Id"] = id;
			p.Exec();
			var ret = p.Parameters["@RETURN_VALUE"].Value;
			int code = ret == null || ret == DBNull.Value ? 0 : Convert.ToInt32(ret);
