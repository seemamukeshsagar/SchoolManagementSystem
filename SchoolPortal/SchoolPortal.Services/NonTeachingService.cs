	...
	public class NonTeachingService : INonTeachingService
	{
		...
		public int Add(NonTeachingMaster entity)
		{
			...
			using (var transaction = _connection.BeginTransaction())
			{
				try
				{
					...
					proc["@Id"] = entity.Id;
					proc["@FirstName"] = entity.FirstName ?? string.Empty;
					proc["@MiddleName"] = entity.MiddleName ?? (object)DBNull.Value;
					proc["@LastName"] = entity.LastName ?? string.Empty;
					proc["@DOB"] = entity.DOB ?? throw new ArgumentException("DOB cannot be null", nameof(entity.DOB));
					proc["@DOJ"] = entity.DOJ ?? throw new ArgumentException("DOJ cannot be null", nameof(entity.DOJ));
		...
		public bool Update(NonTeachingMaster entity)
		{
			...
			using (var transaction = _connection.BeginTransaction())
			{
				try
				{
					...
					proc["@EmergencyContactRelation"] = entity.EmergencyContactRelation ?? (object)DBNull.Value;
					proc["@IsActive"] = entity.IsActive;
					proc["@ModifiedBy"] = entity.ModifiedBy.HasValue && entity.ModifiedBy.Value != Guid.Empty ? entity.ModifiedBy.Value : (object)DBNull.Value;
		...
		private NonTeachingMaster MapToNonTeaching(DataRow r)
		{
			...
			var t = new NonTeachingMaster
			{
				...
				MiddleName = r.Table.Columns.Contains("MiddleName") && r["MiddleName"] != DBNull.Value ? r["MiddleName"]?.ToString() ?? string.Empty : string.Empty,
